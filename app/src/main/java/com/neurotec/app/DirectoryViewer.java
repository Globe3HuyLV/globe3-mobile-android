...
import com.neurotec.app.BaseListActivity;
import com.globe3.tno.g3_mobile.R;
import com.neurotec.view.InfoDialogFragment;
import com.neurotec.widget.BrowseListItem;
import com.neurotec.widget.CustomItemAdapter;
import com.neurotec.widget.FileListItem;
import com.neurotec.widget.HeaderItem;
import com.neurotec.widget.ListItem;
...
public final class DirectoryViewer extends BaseListActivity {
	...
	public static final String ANDROID_ASSET_DESCRIPTOR = "file:///android_asset";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		items.add(new BrowseListItem(getString(R.string.neurotec_title_directory_view_browse)));
		items.add(new HeaderItem(getString(R.string.neurotec_title_item_from_sample_directory)));

		AssetManager am = getResources().getAssets();
		try {
			String[] sampleFiles = am.list(dirName);
			if (sampleFiles.length == 0) {
				items.add(new ListItem(getString(R.string.neurotec_title_directory_view_no_items), null, false));
			} else {
				for (String child : sampleFiles) {
					items.add(new FileListItem(child, combinePath(ANDROID_ASSET_DESCRIPTOR, combinePath(dirName, child))));
				}
			}
		} catch (IOException e) {
		...
		setTitle(getString(R.string.neurotec_title_directory_view_activity) + dirName);
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (selectedItem instanceof BrowseListItem) {
			...
			startActivityForResult(Intent.createChooser(intent, getString(R.string.neurotec_title_input_source_activity)), BROWSE_REQUEST_CODE);
			...
			InfoDialogFragment.newInstance(getString(R.string.neurotec_msg_not_recognized_file_loading_type)).show(getFragmentManager(), "question");
		}
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			String message = getString(R.string.neurotec_msg_third_party_file_loading_app_failed) + " " + getString(R.string.neurotec_msg_dialog_retry_to_open_file);
